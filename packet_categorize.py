...
class LiveCapture:
    def __init__(self, buff):
        ...
        self.response = []
        # 패킷 캡쳐 실행
        cap = pyshark.LiveCapture(interface='2', bpf_filter='tcp and port 102')  # GOOSE - ether proto 0x88B8
        # cap = pyshark.FileCapture('D:/dev/react-web-app/hmi_template/server/packet/mms/fresh.pcap', display_filter="mms or goose")

        cap.apply_on_packets(self.save)  # 각 패킷마다 save 함수 시행
    ...
    def save(self, pkt):
        ...
        try:
            ...
            # redis setter
            # parse_dict = json.dumps(mainDict, ensure_ascii=False, ).encode("utf-8")  # redis 에 encoding 하여 저장
            # r.set("packet" + stamp, parse_dict)
    ...
    def categorizing(self):
        ...
        schedule.add_job(exec_cron, 'cron', second="*")
        schedule.start()  # 시작
        # exec_cron()
